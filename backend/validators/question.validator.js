...
function validateQuestion(req, res, next) {
    ...
    if (!quiz_id) {
        return errorResponse(
            res,
            'VALIDATION_ERROR',
            "L'identifiant du quiz est requis",
            400,
            'quiz_id'
        )
    }
    ...
    if (question_text.length < 10) {
        return errorResponse(
            res,
            'VALIDATION_ERROR',
            'Le texte de la question doit contenir au moins 10 caracteres',
            400,
            'question_text'
        )
    }
    if (question_text.length > 500) {
        return errorResponse(
            res,
            'VALIDATION_ERROR',
            'Le texte de la question ne doit pas depasser 500 caracteres',
            400,
            'question_text'
        )
    }
    ...
    if (!Array.isArray(options)) {
        return errorResponse(
            res,
            'VALIDATION_ERROR',
            'Les options doivent etre un tableau',
            400,
            'options'
        )
    }
    ...
    if (type === 'qcm' && options.length !== 4) {
        return errorResponse(
            res,
            'VALIDATION_ERROR',
            'Un QCM doit avoir exactement 4 options',
            400,
            'options'
        )
    }
    if (type === 'vf' && options.length !== 2) {
        return errorResponse(
            res,
            'VALIDATION_ERROR',
            'Une question Vrai/Faux doit avoir exactement 2 options',
            400,
            'options'
        )
    }
    ...
    if (!correct_answer) {
        return errorResponse(
            res,
            'VALIDATION_ERROR',
            'La reponse correcte est requise',
            400,
            'correct_answer'
        )
    }
    if (!options.includes(correct_answer)) {
        return errorResponse(
            res,
            'VALIDATION_ERROR',
            'La reponse correcte doit etre presente dans les options',
            400,
            'correct_answer'
        )
    }
