...
function createTestApp() {
    ...
    authRouter.post('/register', async (req, res) => {
        ...
        try {
            ...
            const [existingUsers] = await mockPool.query('SELECT id FROM users WHERE email = ?', [
                email
            ])
            ...
            const [users] = await mockPool.query(
...
describe('Auth API', () => {
    ...
    describe('GET /api/auth/me', () => {
        it("devrait retourner le profil de l'utilisateur connecte", async () => {
            const token = jwt.sign({ userId: 1, role: 'prof' }, process.env.JWT_SECRET, {
                expiresIn: '24h'
            })
