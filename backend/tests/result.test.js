...
function createTestApp() {
    ...
    resultRouter.post('/', authenticateToken, requireEleve, async (req, res) => {
        try {
            ...
            if (typeof score !== 'number' || score < 0) {
                return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: 'Score invalide', field: 'score' } });
            }
            ...
            const [result] = await mockPool.query('INSERT INTO results ...', [req.user.userId, quiz_id, score]);
            const [results] = await mockPool.query('SELECT * FROM results WHERE id = ?', [result.insertId]);
    ...
    resultRouter.get('/me', authenticateToken, requireEleve, async (req, res) => {
        try {
            const [results] = await mockPool.query('SELECT r.id, r.score, r.played_at, q.title as quiz_title FROM results r JOIN quizzes q ON r.quiz_id = q.id WHERE r.user_id = ? ORDER BY r.played_at DESC', [req.user.userId]);
    ...
    resultRouter.get('/quiz/:quizId', authenticateToken, requireProf, async (req, res) => {
        try {
            ...
            const [results] = await mockPool.query('SELECT r.id, r.score, r.played_at, u.email as student_email FROM results r JOIN users u ON r.user_id = u.id WHERE r.quiz_id = ? ORDER BY r.played_at DESC', [req.params.quizId]);
            return res.status(200).json({ success: true, data: results });
        } catch (error) {
            return res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Erreur interne' } });
        }
...
describe('Result API', () => {
    ...
    describe('POST /api/results', () => {
        it('devrait enregistrer un score pour un eleve', async () => {
            ...
            mockQuery
                ...
                .mockResolvedValueOnce([[{ id: 1, user_id: 2, quiz_id: 1, score: 8, played_at: new Date() }]]);

            const res = await request(app).post('/api/results').set('Authorization', `Bearer ${token}`).send({ quiz_id: 1, score: 8 });
        ...
        it('devrait rejeter un score negatif', async () => {
            const token = getEleveToken();
            const res = await request(app).post('/api/results').set('Authorization', `Bearer ${token}`).send({ quiz_id: 1, score: -5 });
        ...
        it('devrait rejeter si le quiz n\'existe pas', async () => {
            ...
            const res = await request(app).post('/api/results').set('Authorization', `Bearer ${token}`).send({ quiz_id: 999, score: 8 });
    ...
    describe('GET /api/results/quiz/:quizId', () => {
        it('devrait retourner les resultats d\'un quiz pour le prof', async () => {
            ...
            const res = await request(app).get('/api/results/quiz/1').set('Authorization', `Bearer ${token}`);
        ...
        it('devrait retourner 404 si le quiz n\'existe pas', async () => {
            ...
            const res = await request(app).get('/api/results/quiz/999').set('Authorization', `Bearer ${token}`);
