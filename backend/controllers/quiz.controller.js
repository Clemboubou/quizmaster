...
async function createQuiz(req, res) {
    try {
        ...
        while (codeExists) {
            ...
            const [existing] = await pool.query('SELECT id FROM quizzes WHERE access_code = ?', [
                accessCode
            ])
        ...
        const [quizzes] = await pool.query('SELECT * FROM quizzes WHERE id = ?', [result.insertId])
...
async function updateQuiz(req, res) {
    try {
        ...
        const [updatedQuizzes] = await pool.query('SELECT * FROM quizzes WHERE id = ?', [id])
